...
public class DBHandler
{
		...
		try
		{
			...
			for(int x : randList)
			{
                if(!ss.wholeList)
                {
                    if(checkLists(x, ss))
                    {
                        ss.message = Messages.getMess56(ss);
                            ss.dbgApp("got here selectList 2A: row "+x+" in Know or Maybe Know");
                        continue;
                    }
                }
	...
	public static void saveState(SessionState ss) // throws SQLException, ServletException
	{
		...
		ss.setLists(); // IMPORTANT: may no longer be used
		ss.stringScores(); // sets this.scores to a csv of study data
		...
		updateScores(ss); // updates userlist column scores for ss.userID
	} // creates a csv of study data, updates the lists table and updates userlist column scores for ss.userID
	...
	} // updates userlist column scores for ss.userID
	...
	{	// updates lists' csvlist according to passed userID and name, and ss.language
		try
		{
