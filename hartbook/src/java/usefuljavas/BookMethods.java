...
public final class BookMethods
{
	...
	public static void handleClear(String clearList, String typ, SessionState ss)
	{
		...
		for(Lists list : EnumSet.range(Lists.MYLIST, Lists.MAYBELIST))
		{
			...
			if(name.equals(ss.listGroup))
			{
				...
				ss.clearMapElem(key, ss); // removes the element at key and updates table lists' csvlist to ""
				return;
			}
		}
		
		ss.setTypes(); // IMPORTANT: may no longer be necessary
		
		ss.message = Messages.getMess23(clearList, ss); // returns a mess of ss.type from clearlist cleared
	}
    ...
    public static String prntStdy1(SessionState ss)
    {
		...
		// {"+ss.html.getRollOverVar(i, ss)+"
			if(ss.getMeaning().equals("kanE") || ss.getMeaning().equals("yomE"))
			{
				s = s+"<tr><td onmouseover='handleDisplay("+ss.html.getRollOverVar(i, ss)+")' onmouseout='mouseOut()' ><h2 class='study_word2' >"+wb.eng+"</h2></td></tr>";
			}
			else if(ss.getMeaning().equals("Ekan") || ss.getMeaning().equals("Eyom"))
			{
				if(ss.threeCols)
	...
	private static void setUseList(SessionState ss)
	{
		...
		if(ss.wordList.isEmpty() || ss.wordList == null || ss.wordList.size() == 0)
		{
			//ss.useList = ss.wordList;
				//ss.dbgApp("got to setUseList 2A: ss.wordList was not assigned to ss.useList");
                        //.size() = "+ss.useList.size()+"ss.useList.get(0)="+ss.useList.get(0)); //debug
		...
		else if(ss.wordList.size() <= 50)
		{
			...
			ss.message = Messages.getMess16(ss);
                //ss.dbgApp("got to setUseList 2B: ss.useList.size() = "+ss.useList.size()+"ss.useList.get(0)="+ss.useList.get(0));
		}
		else
		{
            ss.wdlist = true;
